...
public class BatalkanTransaksiAdapter extends RecyclerView.Adapter<BatalkanTransaksiAdapter.BatalkanTransaksiViewholder> {

    Context context;
    ArrayList<HashMap<String, String>> listProdukBatalkan = new ArrayList<>();
    private Pref pref;

    public BatalkanTransaksiAdapter(Context context, ArrayList<HashMap<String, String>> listProdukBatalkan) {
        this.context = context;
        this.listProdukBatalkan = listProdukBatalkan;
    }

    @NonNull
    @Override
    public BatalkanTransaksiViewholder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {

        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.item_produk_rincian_transaksi, parent, false);
        return new BatalkanTransaksiViewholder(view);

    }

    @Override
    public void onBindViewHolder(@NonNull BatalkanTransaksiViewholder holder, int position) {

        HashMap<String, String> item = new HashMap<>();
        item = this.listProdukBatalkan.get(position);
    ...
    public int getItemCount() {
        return (null != listProdukBatalkan ? listProdukBatalkan.size() : 0);
    }
    ...
    public class BatalkanTransaksiViewholder extends RecyclerView.ViewHolder {
        ...
        public BatalkanTransaksiViewholder(@NonNull View itemView) {
            super(itemView);
